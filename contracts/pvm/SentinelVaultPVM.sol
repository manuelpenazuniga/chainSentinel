...
// ============================================================================
// SentinelVaultPVM — PolkaVM-compatible variant of SentinelVault
// ============================================================================
//
// This contract is FUNCTIONALLY IDENTICAL to SentinelVault.sol.
// It exists solely to be compiled with `resolc` (the Parity Solidity→PVM
// compiler), which targets PolkaVM instead of the EVM.
//
// The only source-level differences from SentinelVault.sol:
//
//   1. No OpenZeppelin imports.
//      OpenZeppelin's SafeERC20 uses inline assembly (YUL) that resolc may
//      reject or handle differently. This file replaces it with explicit
//      return-value-checked ERC-20 calls — semantically equivalent for
//      well-behaved tokens.
//
//   2. Inline IERC20 interface.
//      We need only `transfer` and `transferFrom`. Inlining avoids resolc
//      having to locate the OZ source tree.
//
//   3. Inline ReentrancyGuard.
//      OZ's implementation is a pure storage-slot pattern with no assembly
//      (verified in OZ v5). We inline it here to keep the file self-contained.
//
// The public ABI (functions, events, custom errors, constructor) is 100%
// identical to SentinelVault.sol, so the agent and frontend need NO changes.
//
// Compilation:
//   resolc contracts/pvm/SentinelVaultPVM.sol \
//     --abi --bin --output-dir contracts/out/pvm/
//
// Prohibited opcodes NOT used here (verified):
//   SELFDESTRUCT — not present
//   EXTCODECOPY  — not present
...
contract SentinelVaultPVM {
    // ─── Inline ReentrancyGuard ───────────────────────────────────────────────
    // Equivalent to OpenZeppelin ReentrancyGuard v5. No assembly required.
    ...
    /// @notice Creates a new SentinelVaultPVM
